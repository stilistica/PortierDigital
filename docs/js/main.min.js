/*
 * ATTENTION: The "eval" devtool has been used (maybe by default in mode: "development").
 * This devtool is neither made for production nor for readable output files.
 * It uses "eval()" calls to create a separate source file in the browser devtools.
 * If you are trying to read the output file, select a different devtool (https://webpack.js.org/configuration/devtool/)
 * or disable the default devtool with "devtool: false".
 * If you are looking for production-ready output files, see mode: "production" (https://webpack.js.org/configuration/mode/).
 */
/******/ (() => { // webpackBootstrap
/******/ 	"use strict";
/******/ 	var __webpack_modules__ = ({

/***/ "./src/js/main.js":
/*!************************!*\
  !*** ./src/js/main.js ***!
  \************************/
/***/ ((__unused_webpack___webpack_module__, __webpack_exports__, __webpack_require__) => {

eval("{__webpack_require__.r(__webpack_exports__);\n/* harmony import */ var _modules_header_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./modules/header.js */ \"./src/js/modules/header.js\");\n/* harmony import */ var _modules_works_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./modules/works.js */ \"./src/js/modules/works.js\");\n/* harmony import */ var _modules_testimonials_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./modules/testimonials.js */ \"./src/js/modules/testimonials.js\");\n/* harmony import */ var _modules_values_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./modules/values.js */ \"./src/js/modules/values.js\");\n/* harmony import */ var _modules_companies_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./modules/companies.js */ \"./src/js/modules/companies.js\");\n/* harmony import */ var _modules_blog_js__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./modules/blog.js */ \"./src/js/modules/blog.js\");\n\n\n\n\n\n\n\n//# sourceURL=webpack://portierdigital/./src/js/main.js?\n}");

/***/ }),

/***/ "./src/js/modules/blog.js":
/*!********************************!*\
  !*** ./src/js/modules/blog.js ***!
  \********************************/
/***/ ((__unused_webpack___webpack_module__, __webpack_exports__, __webpack_require__) => {

eval("{__webpack_require__.r(__webpack_exports__);\nvar blogArr = [{\n  mobUrl: \"img/blog/card-1-mob.webp\",\n  tabletUrl: \"img/blog/card-1-tablet.webp\",\n  deskUrl: \"img/blog/card-1-desk.webp\",\n  title: \"Free fonts alternatives: The ultimate <br/> guide\",\n  desc: \"You must engrave deeply in your mind and never forget: your emotional commitment to what you are doing will be translated into your work.\"\n}, {\n  mobUrl: \"img/blog/card-2-mob.webp\",\n  tabletUrl: \"img/blog/card-2-tablet.webp\",\n  deskUrl: \"img/blog/card-2-desk.webp\",\n  title: \"How to design an app from stratch\",\n  desc: \"You must engrave deeply in your mind and never forget: your emotional commitment to what you are doing will be translated into your work.\"\n}, {\n  mobUrl: \"img/blog/card-3-mob.webp\",\n  tabletUrl: \"img/blog/card-3-tablet.webp\",\n  deskUrl: \"img/blog/card-3-desk.webp\",\n  title: \"100 things a UI-UX Designer should <br/> know\",\n  desc: \"You must engrave deeply in your mind and never forget: your emotional commitment to what you are doing will be translated into your work.\"\n}, {\n  mobUrl: \"img/blog/card-4-mob.webp\",\n  tabletUrl: \"img/blog/card-4-tablet.webp\",\n  deskUrl: \"img/blog/card-4-desk.webp\",\n  title: \"4 rules for intuitive UX\",\n  desc: \"You must engrave deeply in your mind and never forget: your emotional commitment to what you are doing will be translated into your work.\"\n}];\nvar blogContainer = document.querySelector(\".blog__cards\");\nif (blogContainer) {\n  var cardsHtmlBlog = blogArr.map(function (_ref) {\n    var mobUrl = _ref.mobUrl,\n      tabletUrl = _ref.tabletUrl,\n      deskUrl = _ref.deskUrl,\n      title = _ref.title,\n      desc = _ref.desc;\n    return \"\\n\\t\\t<li class=\\\"blog__card\\\">\\n\\t\\t<picture class=\\\"blog__card-image\\\">\\n      <source srcset=\\\"\".concat(deskUrl, \"\\\" media=\\\"(min-width: 1440px)\\\">\\n      <source srcset=\\\"\").concat(tabletUrl, \"\\\" media=\\\"(min-width: 768px)\\\">\\n      <img src=\\\"\").concat(mobUrl, \"\\\" alt=\\\"Work \").concat(title, \"\\\">\\n    </picture>\\n\\t\\t<div class=\\\"blog__card-info\\\">\\n\\t\\t<h3>\").concat(title, \"</h3>\\n\\t\\t<p class=\\\"blog__card-text\\\">\").concat(desc, \"<p>\\n\\t\\t<a href=\\\"text.html\\\" class=\\\"blog__card-more\\\" aria-label=\\\"Read full post\\\">\\n\\t\\t  Read full post\\n      <svg>\\n        <use href=\\\"img/sprite.svg#icon-right\\\"></use>\\n      </svg>\\n    </a>\\n\\t\\t</div>\\n\\t\\t</li>\");\n  });\n  blogContainer.innerHTML = cardsHtmlBlog.join(\"\");\n}\n\n//# sourceURL=webpack://portierdigital/./src/js/modules/blog.js?\n}");

/***/ }),

/***/ "./src/js/modules/companies.js":
/*!*************************************!*\
  !*** ./src/js/modules/companies.js ***!
  \*************************************/
/***/ ((__unused_webpack___webpack_module__, __webpack_exports__, __webpack_require__) => {

eval("{__webpack_require__.r(__webpack_exports__);\nvar companiesArr = [{\n  svg: \"img/sprite.svg#icon-m\",\n  title: \"Modulz\",\n  desc: \"Lead Designer\"\n}, {\n  svg: \"img/sprite.svg#icon-ball\",\n  title: \"Stitches\",\n  desc: \"UI Designer\"\n}, {\n  svg: \"img/sprite.svg#icon-delta\",\n  title: \"Vercel\",\n  desc: \"Frontend developer\"\n}, {\n  svg: \"img/sprite.svg#icon-boltm\",\n  title: \"Framer\",\n  desc: \"UX Designer\"\n}, {\n  svg: \"img/sprite.svg#icon-figma\",\n  title: \"Figma\",\n  desc: \"Lead Developer\"\n}];\nvar companiesContainer = document.querySelector(\".companies__cards\");\nif (companiesContainer) {\n  var companiesHtmlValues = companiesArr.map(function (_ref) {\n    var svg = _ref.svg,\n      title = _ref.title,\n      desc = _ref.desc;\n    return \"\\n\\t\\t\\t<div class=\\\"companies__card\\\">\\n\\t\\t  <div class=\\\"companies__card-svg\\\">\\n\\t\\t\\t<svg>\\n        <use href=\\\"\".concat(svg, \"\\\"></use>\\n      </svg></div>\\n\\t\\t\\t<div class=\\\"companies__card-text\\\">\\n\\t\\t\\t\\t  <h3>\").concat(title, \"</h3>\\n\\t\\t\\t\\t\\t<p>\").concat(desc, \"</p>\\n\\t\\t\\t</div>\\n\\t\\t</div>\\n\\t\\t\\t\");\n  });\n  companiesContainer.innerHTML = companiesHtmlValues.join(\"\");\n}\n\n//# sourceURL=webpack://portierdigital/./src/js/modules/companies.js?\n}");

/***/ }),

/***/ "./src/js/modules/header.js":
/*!**********************************!*\
  !*** ./src/js/modules/header.js ***!
  \**********************************/
/***/ ((__unused_webpack___webpack_module__, __webpack_exports__, __webpack_require__) => {

eval("{__webpack_require__.r(__webpack_exports__);\nvar burgerMenu = document.querySelector(\".header__menu-burger\");\nvar navMenu = document.querySelector(\".header__nav\");\nvar burgerUse = burgerMenu.querySelector(\"use\");\nif (burgerMenu && navMenu && burgerUse) {\n  burgerMenu.addEventListener(\"click\", function () {\n    navMenu.classList.toggle(\"active\");\n    if (navMenu.classList.contains(\"active\")) {\n      document.body.style.overflow = \"hidden\";\n      burgerUse.setAttribute(\"href\", \"./img/sprite.svg#icon-close\");\n    } else {\n      document.body.style.overflow = \"\";\n      burgerUse.setAttribute(\"href\", \"./img/sprite.svg#icon-menu\");\n    }\n  });\n  document.querySelectorAll(\".header__nav-link\").forEach(function (el) {\n    el.addEventListener(\"click\", function () {\n      navMenu.classList.remove(\"active\");\n      document.body.style.overflow = \"\";\n      burgerUse.setAttribute(\"href\", \"/img/sprite.svg#icon-menu\");\n    });\n  });\n}\nvar navLinks = document.querySelectorAll(\".header__nav-link\");\nif (navLinks) {\n  var currentPage = window.location.href.split(\"/\").pop();\n  navLinks.forEach(function (link) {\n    var linkPage = link.getAttribute(\"href\");\n    if (linkPage === currentPage) {\n      link.classList.add(\"active-link\");\n    }\n  });\n}\n\n//# sourceURL=webpack://portierdigital/./src/js/modules/header.js?\n}");

/***/ }),

/***/ "./src/js/modules/testimonials.js":
/*!****************************************!*\
  !*** ./src/js/modules/testimonials.js ***!
  \****************************************/
/***/ ((__unused_webpack___webpack_module__, __webpack_exports__, __webpack_require__) => {

eval("{__webpack_require__.r(__webpack_exports__);\nvar testimonialsArr = [{\n  svg: \"img/home/avatar-1.svg\",\n  comment: \"“In fact, whenever you must learn a new skill or alter your career path later in life, you reconnect with that youthful, adventurous part of yourself.”\",\n  name: \"Domingo Menenses\",\n  job: \"Cobrand - CEO\"\n}, {\n  svg: \"img/home/avatar-2.svg\",\n  comment: \"“In fact, whenever you must learn a new skill or alter your career path later in life, you reconnect with that youthful, adventurous part of yourself.”\",\n  name: \"Karla Benivez\",\n  job: \"Tinnie - CO-Founder\"\n}, {\n  svg: \"img/home/avatar-3.svg\",\n  comment: \"“In fact, whenever you must learn a new skill or alter your career path later in life, you reconnect with that youthful, adventurous part of yourself.”\",\n  name: \"Mark Clark\",\n  job: \"Airbnb - Lead Designer\"\n}];\nvar testimonialsContainer = document.querySelector(\".testimonials__cards\");\nif (testimonialsContainer) {\n  var testimonialsHtml = testimonialsArr.map(function (_ref) {\n    var svg = _ref.svg,\n      comment = _ref.comment,\n      name = _ref.name,\n      job = _ref.job;\n    return \"\\n\\t\\t<div class=\\\"testimonials__card\\\">\\n\\t\\t  <div class=\\\"testimonials__card-svg\\\">\\n\\t\\t\\t<svg>\\n        <use href=\\\"\".concat(svg, \"\\\"></use>\\n      </svg></div>\\n\\t\\t\\t<div class=\\\"testimonials__card-text\\\">\\n\\t\\t\\t  <p class=\\\"testimonials__card-text-com\\\">\").concat(comment, \"</p>\\n\\t\\t\\t\\t<div class=\\\"testimonials__card-text-info\\\">\\n\\t\\t\\t\\t  <h3>\").concat(name, \"</h3>\\n\\t\\t\\t\\t\\t<p>\").concat(job, \"</p>\\n\\t\\t\\t\\t</div>\\n\\t\\t\\t</div>\\n\\t\\t</div>\\n\\t\\t\");\n  });\n  testimonialsContainer.innerHTML = testimonialsHtml.join(\"\");\n}\n\n//# sourceURL=webpack://portierdigital/./src/js/modules/testimonials.js?\n}");

/***/ }),

/***/ "./src/js/modules/values.js":
/*!**********************************!*\
  !*** ./src/js/modules/values.js ***!
  \**********************************/
/***/ ((__unused_webpack___webpack_module__, __webpack_exports__, __webpack_require__) => {

eval("{__webpack_require__.r(__webpack_exports__);\nvar valuesArr = [{\n  svg: \"img/sprite.svg#icon-smile\",\n  title: \"Passionate\",\n  desc: \"As Pasteur himself commented, “Chance favors only the prepared mind.\"\n}, {\n  svg: \"img/sprite.svg#icon-fur\",\n  title: \"Curious\",\n  desc: \"As Pasteur himself commented, “Chance favors only the prepared mind.\"\n}, {\n  svg: \"img/sprite.svg#icon-planet\",\n  title: \"Team Work\",\n  desc: \"As Pasteur himself commented, “Chance favors only the prepared mind.\"\n}, {\n  svg: \"img/sprite.svg#icon-sm\",\n  title: \"Discipline\",\n  desc: \"As Pasteur himself commented, “Chance favors only the prepared mind.\"\n}, {\n  svg: \"img/sprite.svg#icon-magic\",\n  title: \"Creative\",\n  desc: \"As Pasteur himself commented, “Chance favors only the prepared mind.\"\n}, {\n  svg: \"img/sprite.svg#icon-person\",\n  title: \"Compassionate\",\n  desc: \"As Pasteur himself commented, “Chance favors only the prepared mind.\"\n}];\nvar valuesContainer = document.querySelector(\".values__cards\");\nif (valuesContainer) {\n  var cardsHtmlValues = valuesArr.map(function (_ref) {\n    var svg = _ref.svg,\n      title = _ref.title,\n      desc = _ref.desc;\n    return \"\\n\\t\\t\\t<div class=\\\"card\\\">\\n        <svg class=\\\"card__svg\\\">\\n          <use href=\\\"\".concat(svg, \"\\\"></use>\\n        </svg>\\n        <div class=\\\"card__info\\\">\\n          <h2 class=\\\"card__info-title\\\">\").concat(title, \"</h2>\\n          <p class=\\\"card__info-text\\\">\").concat(desc, \"</p>\\n        </div>\\n      </div> \\n\\t\\t\\t\");\n  });\n  valuesContainer.innerHTML = cardsHtmlValues.join(\"\");\n}\n\n//# sourceURL=webpack://portierdigital/./src/js/modules/values.js?\n}");

/***/ }),

/***/ "./src/js/modules/works.js":
/*!*********************************!*\
  !*** ./src/js/modules/works.js ***!
  \*********************************/
/***/ ((__unused_webpack___webpack_module__, __webpack_exports__, __webpack_require__) => {

eval("{__webpack_require__.r(__webpack_exports__);\nvar worksArr = [{\n  mobUrl: \"img/works/card-1-mob.webp\",\n  tabletUrl: \"img/works/card-1-tablet.webp\",\n  deskUrl: \"img/works/card-1-desk.webp\",\n  desc: \"Portier - The ultimate templates & components library.\"\n}, {\n  mobUrl: \"img/works/card-2-mob.webp\",\n  tabletUrl: \"img/works/card-2-tablet.webp\",\n  deskUrl: \"img/works/card-2-desk.webp\",\n  desc: \"The Coporate - Card for Indian Startups\"\n}, {\n  mobUrl: \"img/works/card-3-mob.webp\",\n  tabletUrl: \"img/works/card-3-tablet.webp\",\n  deskUrl: \"img/works/card-3-desk.webp\",\n  desc: \"Components Web App Interface UI\"\n}];\nvar worksContainer = document.querySelector(\".works\");\nif (worksContainer) {\n  var cardsHtmlWorks = worksArr.map(function (_ref) {\n    var mobUrl = _ref.mobUrl,\n      tabletUrl = _ref.tabletUrl,\n      deskUrl = _ref.deskUrl,\n      desc = _ref.desc;\n    return \"\\n\\t\\t<li class=\\\"works__card\\\">\\n\\t\\t<picture class=\\\"works__card-image\\\">\\n      <source srcset=\\\"\".concat(deskUrl, \"\\\" media=\\\"(min-width: 1440px)\\\">\\n      <source srcset=\\\"\").concat(tabletUrl, \"\\\" media=\\\"(min-width: 568px)\\\">\\n      <img src=\\\"\").concat(mobUrl, \"\\\" alt=\\\"Work \").concat(desc, \"\\\">\\n    </picture>\\n\\t\\t<p class=\\\"works__card-text\\\">\").concat(desc, \"<p>\\n\\t\\t<a class=\\\"works__card-more\\\" aria-label=\\\"Open Project\\\">\\n      <svg>\\n        <use href=\\\"img/sprite.svg#icon-more\\\"></use>\\n      </svg>\\n    </a>\\n\\t\\t</li>\");\n  });\n  worksContainer.innerHTML = cardsHtmlWorks.join(\"\");\n}\n\n//# sourceURL=webpack://portierdigital/./src/js/modules/works.js?\n}");

/***/ })

/******/ 	});
/************************************************************************/
/******/ 	// The module cache
/******/ 	var __webpack_module_cache__ = {};
/******/ 	
/******/ 	// The require function
/******/ 	function __webpack_require__(moduleId) {
/******/ 		// Check if module is in cache
/******/ 		var cachedModule = __webpack_module_cache__[moduleId];
/******/ 		if (cachedModule !== undefined) {
/******/ 			return cachedModule.exports;
/******/ 		}
/******/ 		// Create a new module (and put it into the cache)
/******/ 		var module = __webpack_module_cache__[moduleId] = {
/******/ 			// no module.id needed
/******/ 			// no module.loaded needed
/******/ 			exports: {}
/******/ 		};
/******/ 	
/******/ 		// Execute the module function
/******/ 		__webpack_modules__[moduleId](module, module.exports, __webpack_require__);
/******/ 	
/******/ 		// Return the exports of the module
/******/ 		return module.exports;
/******/ 	}
/******/ 	
/************************************************************************/
/******/ 	/* webpack/runtime/make namespace object */
/******/ 	(() => {
/******/ 		// define __esModule on exports
/******/ 		__webpack_require__.r = (exports) => {
/******/ 			if(typeof Symbol !== 'undefined' && Symbol.toStringTag) {
/******/ 				Object.defineProperty(exports, Symbol.toStringTag, { value: 'Module' });
/******/ 			}
/******/ 			Object.defineProperty(exports, '__esModule', { value: true });
/******/ 		};
/******/ 	})();
/******/ 	
/************************************************************************/
/******/ 	
/******/ 	// startup
/******/ 	// Load entry module and return exports
/******/ 	// This entry module can't be inlined because the eval devtool is used.
/******/ 	var __webpack_exports__ = __webpack_require__("./src/js/main.js");
/******/ 	
/******/ })()
;